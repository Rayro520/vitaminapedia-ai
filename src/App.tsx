      ...
      <main className="max-w-7xl mx-auto px-4 py-6 pb-24">
        {activeTab === 'inicio' && (
          <div className="space-y-8 animate-fade-in">
            ...
            {/* PWA launch center */}
            <div className={`p-6 rounded-3xl border text-left mt-8 ${darkMode ? 'bg-slate-900/40 border-slate-800' : 'bg-white border-slate-200'} shadow-md`}>
              <div className="flex items-center space-x-3 mb-4">
                <div className="p-2 bg-amber-500/10 text-amber-500 rounded-lg">
                  <Sparkles className="w-6 h-6 animate-pulse" />
                </div>
                <div>
                  <h3 className="font-extrabold text-lg">⚡ Central de Lançamento PWA</h3>
                  <p className="text-xs opacity-75">Gere os instaladores do aplicativo para rodar direto no celular</p>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="font-bold text-sm mb-3 text-emerald-500 dark:text-emerald-400">📍 Passo a passo para testar no celular:</h4>
                  <ul className="space-y-2 text-xs opacity-90 leading-relaxed list-decimal list-inside pl-1">
                    <li>Copie o link público do seu app: <br /><span className="text-[10px] sm:text-xs font-mono select-all bg-indigo-500/10 dark:bg-slate-800/80 px-2 py-1 rounded text-indigo-400 block mt-1 break-all">https://ais-pre-3swqxmzd3xlneooa6c777o-607570129470.us-west2.run.app</span></li>
                    <li>Abra esse link no navegador do celular (<strong>Chrome</strong> no Android ou <strong>Safari</strong> no iPhone).</li>
                    <li><strong>No Android:</strong> Toque no botão "Adicionar à tela de início" na barra inferior ou acesse o menu de 3 pontinhos e toque em "Instalar aplicativo".</li>
                    <li><strong>No iPhone (iOS):</strong> Toque no botão "Compartilhar" (quadrado com seta para cima) e selecione a opção "Adicionar à Tela de Início".</li>
                    <li>Pronto! Ele funcionará como um aplicativo nativo instalado direto no seu celular.</li>
                  </ul>
                </div>

                <div className="flex flex-col justify-between border-t md:border-t-0 md:border-l border-slate-200 dark:border-slate-800 pt-4 md:pt-0 md:pl-6">
                  <div>
                    <h4 className="font-bold text-sm mb-3 text-amber-500 dark:text-amber-400">🛠️ Baixar ícones do App (Para enviar à loja ou testadores):</h4>
                    <p className="text-xs opacity-75 mb-4">
                      Utilize os botões abaixo para gerar e fazer download dos ícones em alta qualidade (.png) do <strong>Vitaminapédia IA</strong>, gerados instantaneamente a partir do vetor original.
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
                      <button
                        onClick={() => downloadIcon(192, 'icon-192.png')}
                        className="px-3 py-2 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold text-xs rounded-xl flex items-center justify-center space-x-1.5 transition-all text-center"
                      >
                        <Upload className="w-3.5 h-3.5 rotate-180" />
                        <span>Baixar Ícone 192x192px</span>
                      </button>
                      <button
                        onClick={() => downloadIcon(512, 'icon-512.png')}
                        className="px-3 py-2 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs rounded-xl flex items-center justify-center space-x-1.5 transition-all text-center"
                      >
                        <Upload className="w-3.5 h-3.5 rotate-180" />
                        <span>Baixar Ícone 512x512px</span>
                      </button>
                    </div>
                  </div>

                  <details className="text-xs">
                    <summary className="cursor-pointer font-bold text-indigo-400 hover:text-indigo-300">Mostrar código do manifesto concluído (manifest.json)</summary>
                    <div className="mt-2 p-3 rounded-lg bg-slate-900 border border-slate-800 text-[10px] font-mono overflow-auto max-h-40 text-slate-300 select-all">
{`{
  "id": "/index.html",
  "name": "Vitaminapedia IA",
  "short_name": "Vitaminapedia",
  "description": "Dicionário e assistente de vitaminas, minerais e nutrição inteligente com Scanner de Rótulos por IA.",
  "lang": "pt",
  "dir": "ltr",
  "start_url": "/",
  "scope": "/",
